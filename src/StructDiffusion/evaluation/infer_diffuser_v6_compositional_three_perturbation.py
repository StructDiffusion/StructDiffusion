...
from StructDiffusion.training.train_diffuser_v6_compositional_three_perturbation import load_model, get_diffusion_variables, extract, get_struct_objs_poses, move_pc_and_create_scene, visualize_batch_pcs
from StructDiffusion.data.dataset_v4_diffuser_compositional_three_perturbation import SemanticArrangementDataset
...
class DiffuserInference:
    ...
    def __init__(self, model_dir, data_split="test", override_data_dirs=None, override_index_dirs=None,
                 invalid_noise_weight=2.0, valid_noise_weight=2.0):
        ...
        self.valid_noise_weight = valid_noise_weight
    ...
    def limited_batch_inference(self, data, num_samples, convert_to_tensors=True, return_numpy=True):
        ...
        with torch.no_grad():
            ...
            raw_null_sentence = []
            null_sentence_pad_mask = copy.deepcopy(sentence_pad_mask)
            for ti, (tv, tt) in enumerate(raw_valid_sentence):
                if tt == "physics":
                    assert tv == "valid"
                    raw_invalid_sentence.append(("invalid", tt))
                    raw_null_sentence.append(("PAD", None))
                    null_sentence_pad_mask[0, ti] = 1.0
                else:
                    raw_invalid_sentence.append((tv, tt))
                    raw_null_sentence.append((tv, tt))
            print("raw valid sentence:", raw_valid_sentence)
            print("raw invalid sentence:", raw_invalid_sentence)
            print("raw null sentence:", raw_null_sentence)
            print("sentence pad mask:", sentence_pad_mask)
            print("null sentence pad mask:", null_sentence_pad_mask)
            invalid_sentence = torch.LongTensor([self.tokenizer.tokenize(*i) for i in raw_invalid_sentence]).to(device, non_blocking=True)
            null_sentence = torch.LongTensor([self.tokenizer.tokenize(*i) for i in raw_null_sentence]).to(device, non_blocking=True)
            ...
            null_sentence = null_sentence.repeat(num_samples, 1)
            sentence_pad_mask = sentence_pad_mask.repeat(num_samples, 1)
            null_sentence_pad_mask = null_sentence_pad_mask.repeat(num_samples, 1)
            ...
            for t_index in tqdm.tqdm(reversed(range(0, noise_schedule.timesteps)), desc='sampling loop time step',
                ...
                # run null
                struct_xyztheta_outputs, obj_xyztheta_outputs = self.model.forward(t, xyzs, obj_xyztheta_inputs,
                                                                              struct_xyztheta_inputs, null_sentence,
                                                                              position_index, struct_position_index,
                                                                              token_type_index, struct_token_type_index,
                                                                              start_token,
                                                                              object_pad_mask, struct_pad_mask,
                                                                              null_sentence_pad_mask)
                predicted_noise_null = torch.cat([struct_xyztheta_outputs, obj_xyztheta_outputs], dim=1)

                # combine valid and invalid
                predicted_noise = (1 + self.invalid_noise_weight - self.valid_noise_weight) * predicted_noise_null - self.invalid_noise_weight * predicted_noise_invalid + self.valid_noise_weight * predicted_noise_valid
...
def run_compositional(model_dir, num_samples=10, invalid_noise_weight=0.2, valid_noise_weight=0.2):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_iter["test"]):
            ...
            null_sentence = torch.LongTensor([tokenizer.tokenize(*i) for i in raw_null_sentence]).to(device, non_blocking=True)
            ...
            # for t_index in tqdm.tqdm(reversed(range(0, 1)), desc='sampling loop time step',total=1):
                ...
                # combine valid and invalid
                predicted_noise = (1 + invalid_noise_weight - valid_noise_weight) * predicted_noise_null - invalid_noise_weight * predicted_noise_invalid + valid_noise_weight * predicted_noise_valid
...
if __name__ == "__main__":
    model_dir = "/home/weiyu/Research/intern/StructDiffuser/experiments/20220910-195659/model"
    # run(model_dir, num_samples=10)
    run_compositional(model_dir, num_samples=10, invalid_noise_weight=2.0, valid_noise_weight=2.0)