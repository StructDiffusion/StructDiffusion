...
class SemanticArrangementDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_roots, index_roots, split, tokenizer,
                 unconditional_probability, perturbation_probability, perturbation_mode, min_translation, max_translation, min_rotation, max_rotation,
        ...
        assert max_num_shape_parameters == 6, "5 shape params + 1 valid/invalid token"
        ...
        assert 0.0<=perturbation_probability<=1.0
        assert 0.0 <=unconditional_probability<= 1.0
    ...
    def get_raw_data(self, idx, inference_mode=False, shuffle_object_index=False):
        ...
        ###################################
        # Important: perturb this example
        create_negative_example = False
        create_unconditional_example = False
        if not inference_mode:
            if np.random.random() < self.perturbation_probability:
                create_negative_example = True
            if np.random.random() < self.unconditional_probability:
                create_unconditional_example = True
        ...
        if create_negative_example:

            _, obj_perturbation_matrices = random_move_obj_xyzs(obj_xyzs,
                                                                ...
                                                                return_moved_obj_idxs=False,
                                                                return_perturbed_obj_xyzs=False)
            perturbed_goal_pc_poses_in_struct = []
            for pm, goal_pc_pose_in_struct in zip(obj_perturbation_matrices, goal_pc_poses_in_struct):
                # note: this is same as perturbing the goal poses of the objects, verified
                perturbed_goal_pc_poses_in_struct.append(goal_pc_pose_in_struct @ pm)

        # code below transform current object point cloud to the goal point cloud in the world frame (more efficiently)
        if self.debug:
            perturbed_new_obj_xyzs = copy.deepcopy(obj_xyzs)
            ...
            for i, obj_xyz in enumerate(obj_xyzs):
                ...
                if create_negative_example:
                    perturbed_goal_pc_pose_in_struct = perturbed_goal_pc_poses_in_struct[i]
                    perturbed_new_obj_xyz = trimesh.transform_points(obj_xyz, structure_pose @ perturbed_goal_pc_pose_in_struct @ np.linalg.inv(current_pc_pose))
                    perturbed_new_obj_xyzs[i] = perturbed_new_obj_xyz

            if create_negative_example:
                show_pcs(new_obj_xyzs + perturbed_new_obj_xyzs, obj_rgbs + new_obj_rgbs, add_coordinate_frame=True)
            else:
        ...
        if structure_parameters["type"] == "circle" or structure_parameters["type"] == "line":
            ...
            if create_unconditional_example:
                ...
                if create_negative_example:
                    sentence.append(("invalid", "physics"))
                else:
                    sentence.append(("valid", "physics"))
                for _ in range(6):
                    sentence_pad_mask.append(0)
        else:
            sentence.append((structure_parameters["type"], "shape"))
        ...
        if self.debug:
            print("---")
            print("negative example?", create_negative_example)
            ...
            print("unconditional example:", create_unconditional_example)
            print("perturbation example:", create_negative_example)
    ...
    def convert_to_tensors(datum, tokenizer):
        ...
        tensors = {
            ...
            "raw_sentence": str(datum["raw_sentence"]),
            "t": datum["t"],
...
if __name__ == "__main__":
    ...
    tokenizer = Tokenizer("/home/weiyu/data_drive/data_new_objects/type_vocabs_coarse_valid.json")

    data_roots = []
    index_roots = []
    for shape, index in [("line", "index_10k")]: # [("circle", "index_34k"), ("line", "index_42k"), ("tower", "index_13k"), ("dinner", "index_24k")]:
