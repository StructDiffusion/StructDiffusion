    ...
    for epoch in range(num_epochs):
        ...
        with tqdm.tqdm(total=len(data_iter["train"])) as pbar:
            for step, batch in enumerate(data_iter["train"]):
                ...
                # remove the position index and token type index for the language part since this model doesn't use it
                remove_num_positions = cfg.dataset.max_num_shape_parameters
                token_type_index = token_type_index[:, remove_num_positions:]
                position_index = position_index[:, remove_num_positions:]
...
def infer_once(cfg, model, batch, device):
    ...
    with torch.no_grad():
        ...
        # input
        # batch_latent_codes = batch_latent_codes.to(device, non_blocking=True)
...
def load_model(model_dir, ngc_vocab=False):
    ...
    model = ACTORVAE3(num_attention_heads=model_cfg.num_attention_heads,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", help='config yaml file',
                        default='../configs/generative_models/actor_vae_3networks_all_shapes.yaml',
                        type=str)
