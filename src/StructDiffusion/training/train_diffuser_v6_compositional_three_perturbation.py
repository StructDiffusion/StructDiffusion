...
def train_model(cfg, model, data_iter, noise_schedule, optimizer, warmup, num_epochs, device, save_best_model,
    ...
    for epoch in range(num_epochs):
        ...
        #     best_score = score
        model_dir = os.path.join(cfg.experiment_dir, "current_model")
        print("Saving model to {}".format(model_dir))
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        save_model(model_dir, cfg, cfg.max_epochs, model, optimizer, scheduler=None)
...
def run_model(cfg):
    ...
    train_dataset = SemanticArrangementDataset(data_roots=data_cfg.dirs,
                                               ...
                                               split="train",
                                               tokenizer=tokenizer,
                                               unconditional_probability=data_cfg.unconditional_probability,
                                               perturbation_probability=data_cfg.perturbation_probability,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", help='config yaml file',
                        default='../../configs/diffuser_v6_compositional_three_perturbation.yaml',
                        type=str)
