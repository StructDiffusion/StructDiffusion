...
from StructDiffusion.data.dataset_v5_collision_discriminator import CollisionDataGenerator
...
from StructDiffusion.models.models import TransformerNoisedClassifier
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm.tqdm(total=len(data_iter["train"])) as pbar:
            for step, batch in enumerate(data_iter["train"]):
                ...
                # Add noise to inputs for the noised classifier
...
def load_guidance_model(model_dir):
    """
    ...
    # initialize model
    model_cfg = cfg.model
    model = TransformerNoisedClassifier(vocab_size,
...
def run_model(cfg):
    ...
    mesh_scene_viewer = MeshSceneViewer(assets_dir=data_cfg.assets_dir, cache_mesh=True)

    train_dataset = CollisionDataGenerator(data_roots=data_cfg.dirs,
                                           ...
                                           num_random_perturbations_per_positive=data_cfg.num_random_perturbations_per_positive,
                                           ...
                                           mesh_scene_viewer=mesh_scene_viewer,
                                           save_dir=None)

    valid_dataset = CollisionDataGenerator(data_roots=data_cfg.dirs,
                                           index_roots=data_cfg.index_dirs,
                                           split="valid",
                                           tokenizer=tokenizer,
                                           min_translation=data_cfg.min_translation,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", help='config yaml file',
                        default='../../../configs/noised_collision_discriminator.yaml',
                        type=str)
