...
# diffusion model
from StructDiffuser.test_diffuser_v3_lang import DiffuserInference

# physics eval
from src.generative_models.try_langevin_actor_vae_3networks_language_all_shapes_discriminator_7 import switch_stdout, visualize_batch_pcs, convert_bool, save_dict_to_h5, move_pc_and_create_scene_new, move_pc, sample_gaussians, fit_gaussians
from brain2.semantic_rearrangement.physics_verification_dinner import verify_datum_in_simulation

# discriminators
from src.physics_eval.eval_vae_cem_lan_local_shape_param_discriminator import DiscriminatorInference
from src.pairwise_collision.test_pairwise_collision_detector import CollisionInference

from sklearn.metrics import confusion_matrix

def evaluate(structure_type, generator_model_dir, data_split, data_root,
             ...
             min_num_samples_either_class=100,
             ...
             override_data_dirs=None, override_index_dirs=None, on_policy_iteration=None):
    ...
    for idx, data_idx in enumerate(prior_dataset_idxs):
        ...
        for b in range(num_batches):
            ...
            new_obj_xyzs, _, subsampled_scene_xyz, _, obj_pair_xyzs = \
                move_pc_and_create_scene_new(obj_xyzs, batch_obj_params, batch_struct_pose, batch_current_pc_pose, target_object_inds, device,
                                             return_scene_pts=discriminator_score_weight>0, return_scene_pts_and_pc_idxs=False, num_scene_pts=discriminator_num_scene_pts, normalize_pc=discriminator_normalize_pc,
        ...
        for goal_pc_pose in best_goal_pc_pose:
            ...
            check, check_dict = verify_datum_in_simulation(d, assets_path, object_model_dir,
                                                           structure_type=structure_type, visualize=False)
        ...
        for bsi in range(len(best_score_so_far)):
            ...
            sample_scores.append(sd["discriminator_score"])
            sample_physics_labels.append(bool(sd["check"]))
            if sd["check"] == False:
        ...
        if summary_writer:
            summaries = {}
            for decision_threshold in [0.5, 0.8]:
                cm = confusion_matrix(sample_physics_labels, np.array(sample_scores) > decision_threshold, labels=[False, True]).ravel()
                summaries["{}_TN".format(decision_threshold)] = cm[0]
                summaries["{}_FP".format(decision_threshold)] = cm[1]
                summaries["{}_FN".format(decision_threshold)] = cm[2]
                summaries["{}_TP".format(decision_threshold)] = cm[3]
            summary_writer.add_scalars("{}/physics_{}_{}_confusion".format(on_policy_iteration, structure_type, data_split), summaries, idx)

        if min(num_negative_samples, num_positive_samples) >= min_num_samples_either_class:
            break
...
if __name__ == "__main__":
    ...
    ###################################################################################################################
    # batch testing for testing objects, collision 0.0, discriminator 1.0
    ###################################################################################################################
    ...
    evaluate(structure_type="dinner",
             ...
             data_root="/home/weiyu/data_drive/physics_eval_diffuser_d1c0_dinner_10k_test_objects_discriminator_local_shape_param_collision_100",
    ...
    evaluate(structure_type="circle",
             generator_model_dir=model_dir, data_split="train",
             data_root="/home/weiyu/data_drive/physics_eval_diffuser_d1c0_circle_10k_test_objects_discriminator_local_shape_param_collision_100",
             discriminator_model_dir=discriminator_model_dir,
             collision_model_dir=collision_model_dir,
             test_specific_shape=None,
             collision_score_weight=0.0, discriminator_score_weight=1.0,
    ...
    evaluate(structure_type="line",
             generator_model_dir=model_dir, data_split="train",
             data_root="/home/weiyu/data_drive/physics_eval_diffuser_d1c0_line_10k_test_objects_discriminator_local_shape_param_collision_100",
             discriminator_model_dir=discriminator_model_dir,
             collision_model_dir=collision_model_dir,
             test_specific_shape=None,
             collision_score_weight=0.0, discriminator_score_weight=1.0,
    ...
    evaluate(structure_type="tower",
             generator_model_dir=model_dir, data_split="train",
             data_root="/home/weiyu/data_drive/physics_eval_diffuser_d1c0_stacking_10k_test_objects_discriminator_local_shape_param_collision_100",
             discriminator_model_dir=discriminator_model_dir,
             collision_model_dir=collision_model_dir,
             test_specific_shape=None,
             collision_score_weight=0.0, discriminator_score_weight=1.0,
