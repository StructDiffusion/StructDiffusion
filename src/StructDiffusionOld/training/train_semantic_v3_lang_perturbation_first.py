...
# from StructDiffusion.data.dataset_v41_discriminator_env_lan_local_shape_param_return_pose import SemanticArrangementDataset
...
from StructDiffusion.models.models import TransformerDiffuserSemanticCls
...
def make_negative(x_noisy):
    # Do two types of augmentation
    B = x_noisy.shape[0]
    split = B // 2

    #  add gaussian noise to the inputs
    x_noisy_first = x_noisy[:split]

    x_noise_add = torch.randn_like(x_noisy_first) * 0.05
    x_noise_add = x_noisy_first + x_noise_add

    # or shuffle one object between objects
    x_noisy_second = x_noisy[split:]

    # Randomly perturb one index
    nelem = x_noisy_second.size(1)
    idx = random.randint(0, nelem - 1)

    x_select = x_noisy_second[:, idx:idx+1]
    x_select_roll = torch.roll(x_select, 1, dims=0)

    x_noisy_roll = torch.cat([x_noisy_second[:, :idx], x_select_roll, x_noisy_second[:, idx+1:]], dim=1)

    x_noisy = torch.cat([x_noise_add, x_noisy_roll], dim=0)

    x_noisy = x_noisy[torch.randperm(x_noisy.size(0))]
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm.tqdm(total=len(data_iter["train"])) as pbar:
            for step, batch in enumerate(data_iter["train"]):
                ...
                x_start_negative = make_negative(x_start)

                N = x_start.shape[0]
                split_idx = N // 2
                x_start = torch.cat([x_start[:split_idx], x_start_negative[split_idx:]], dim=0)
                labels = torch.cat([torch.ones(N)[:split_idx], torch.zeros(N)[split_idx:]], dim=0)
                ...
                logits = model.forward(t, xyzs, obj_xyztheta_inputs, struct_xyztheta_inputs, sentence,
                ...
                labels = labels.to(logits.device)[:, None]
    ...
    with torch.no_grad():
        ...
        with tqdm.tqdm(total=len(data_iter["valid"])) as pbar:
            for step, batch in enumerate(data_iter["valid"]):
                ...
                x_noisy_negative = make_negative(x_noisy)

                N = x_noisy.shape[0]
                split_idx = N // 2

                x_noisy = torch.cat([x_noisy[:split_idx], x_noisy_negative[split_idx:]], dim=0)
                labels = torch.cat([torch.ones(N)[:split_idx], torch.zeros(N)[split_idx:]], dim=0)
...
def load_model(model_dir):
    ...
    # initialize model
    model_cfg = cfg.model
    model = TransformerDiffuserSemanticCls(vocab_size,
